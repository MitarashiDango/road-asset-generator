...
namespace MitarashiDango.RoadAssetGenerator
{
    ...
    /// Backward = V- (対向)。減速マークの Slant 自動反転に利用される。
    /// </summary>
    ...
    public class LineStyle
    {
        ...
        // Diamond 専用パラメータ。V 軸方向に繰り返される斜行した平行四辺形。
        // 既定: Diamond Size 1.0 m + Diamond Spacing 1.0 m = 2.0 m 周期。タイル長 10 m を 5 等分する。
        ...
        /// <summary>
        /// このストロークが高さマップ (およびそれを介して法線マップ) に与える塗装厚みの倍率。
    ...
    /// <summary>
    /// レーン間 (またはレーンと路側帯の間) の境界線。任意本数のストロークを保持し、
    /// 隣接ストローク間の中心間隔 (spacingsMeters) を明示的に持つ。
    /// </summary>
    [Serializable]
    public class LineConfig
    {
        ...
        // この境界に描かれるストローク群。U 軸方向に左から右の物理順で並ぶ。
        public List<LineStyle> strokes = new List<LineStyle>();

        // 隣接ストローク間の中心間隔。要素数は strokes.Count - 1 となる。
        ...
        /// <summary>
        /// この境界線が U 軸方向に占める範囲 (スロット) を計算する。配置軸は
        /// ストロークを置く位置 (= ベイカーが利用する境界座標)。ストローク幅が左右対称なら
        /// <paramref name="leftHalf"/> と <paramref name="rightHalf"/> は等しい。
        /// </summary>
        /// <param name="leftHalf">スロット左端から配置軸までの距離。</param>
        /// <param name="rightHalf">配置軸からスロット右端までの距離。</param>
        /// <param name="slotWidth">スロットが U 軸方向に占める総幅。</param>
    ...
    public class LaneConfig
    {
        ...
        /// <summary>
        /// V 軸方向の進行方向。減速マークに影響し、Backward レーンでは Slant が自動反転されるため、
        /// シェブロンは常に進行方向を指す。
        /// </summary>
        ...
        // 例: 全体 0.2 + レーン 0.3 → このレーンの実効摩耗強度は 0.5 (最大 1.0)。
        ...
        // ---- 減速帯 (Rumble Strips) ----
        // 既定: Stripe Thickness 0.30 m + Stripe Spacing 0.70 m = 1.0 m 周期。タイル長 10 m を 10 等分する。
        // 路面凹凸構造指針の標準寸法に準拠。
        ...
        // V 軸の位相オフセット。Spacing / 2 を既定値にして、パターンをタイル内で中央寄せにする。
        ...
        // 既定: Dot Height 1.0 m + Dot Spacing 1.0 m = 2.0 m 周期。タイル長 10 m を 5 等分する。
        ...
        // ---- 減速マーク (山形マーク / Deceleration Marks) ----
        // レーン中央に進行方向を指す V 字型マークを V 軸方向へ周期的に配置する。
        // 急カーブや追突事故多発区間など、減速を要する区間およびその手前に設置される塗装路面標示。
        // 既定: Mark Height 1.0 m + Mark Spacing 4.0 m = 5.0 m 周期。タイル長 10 m を 2 等分する。
        // Line Thickness 0.20 m は政令の標準型に準拠。
    ...
    public class WeatheringConfig
    {
        ...
        // タイヤ跡によるレーン中央付近の劣化強度。レーンごとの追加調整は LaneConfig.tireTrackWearBoost で行う。
        [Range(0f, 1f)] public float tireTrackWear = 0.0f;
        // タイヤ跡が路面標示 (境界線・減速マーク等) に与える摩耗の強度。
        // 0 = 標示はタイヤ跡の影響を受けない、1 = タイヤ跡上で標示が完全に下地色へ近づく。
    ...
    public class RoadConfig
    {
        ...
        {
            // 4 車線・中央二重黄実線、車線間白破線。日本の左側通行を想定して左側 2 車線を Forward にする。
        ...
        {
            // 車線なし道路：区画線なし (住宅地の生活道路や農道など)。走行領域のみ。
